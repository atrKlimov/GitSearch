...
public final class CustomNetworkLoggerPlugin: PluginType {
    ...
    public init(verbose: Bool = false, cURL: Bool = false, output: ((_ separator: String, _ terminator: String, _ items: Any...) -> Void)? = nil, requestDataFormatter: ((Data) -> (String))? = nil, responseDataFormatter: ((Data) -> (Data))? = nil) {
        ...
        self.output = output ?? CustomNetworkLoggerPlugin.reversedPrint
    ...
    fileprivate func outputItems(_ items: [Any]) {
        if isVerbose {
...
private extension CustomNetworkLoggerPlugin {
    ...
    func logNetworkResponse(_ response: HTTPURLResponse?, data: Data?, target: TargetType) -> [Any] {
...
fileprivate extension CustomNetworkLoggerPlugin {
